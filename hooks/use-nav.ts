...
const useOrganization = () => ({ organization: null as any, membership: null as any });
...
export function useFilteredNavItems(items: NavItem[]) {
  ...
  const activeRole = authUser?.roles.some(r => r === 'admin' || r === 'super_admin')
    ? 'admin'
    : authUser?.roles.includes('tutor')
    ? 'tutor'
    : 'parent';
  ...
  const filteredItems = useMemo(() => {
    ...
    return items
      .filter((item) => {
        // Simulated role access control for testing
        if (item.url.startsWith('/admin') && currentRole !== 'admin') {
          return false;
        }
      ...
      .map((item) => {
        ...
        if (item.items && item.items.length > 0) {
          const filteredChildren = item.items.filter((childItem) => {
            // Simulated role access control for testing children
            if (childItem.url.startsWith('/admin') && currentRole !== 'admin') {
              return false;
            }
  ...
  }, [items, accessContext, activeRole]);
